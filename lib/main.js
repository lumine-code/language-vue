...
// The right-hand column is a *language string*, not a scope: it is matched
// against every Tree-sitter grammar's `injectionRegex`, longest match wins.
//
// `jsx` has no grammar of its own here, but tree-sitter-javascript parses JSX,
// so it resolves to `javascript` rather than being dropped.
...
// `less`, `postcss` and indented `sass` have no Tree-sitter grammar in the
// ecosystem, and a TextMate grammar cannot be injected into a Tree-sitter one.
// `less` and `postcss` are CSS supersets close enough that SCSS reads them
// without falling apart, which is the same call nvim-treesitter's Vue queries
// make. Indented `sass` is *not* — it has no braces and no semicolons — so it
// is deliberately absent and its block stays unhighlighted rather than
// mis-highlighted. `stylus` likewise.
